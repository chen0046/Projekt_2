...
void shuffleRandom(); /* Random shuffle */
void splitDeck();   /* Split shuffle */ 
void clearGame(); /* Reset game */
void startGame(); /* Start game */
void basicLD(); /* Loader basic */
void initializeDeck(); /* Initialiserer piles før kort lægges i */
void dealCards();  /* Dealer kort ud i 7 bunker */
void saveDeck();   /* Gemmer decket */
void quitCurrentGame(); /* Fjerner alle aktuelle piles og kort */
void quitGame(); /* Lukker progrmamet */
void helpMessage(); /* Hjælpe besked ifm. validering af brugerinput */
void invalidInput(); /* Fejlhåndtering af brugerinput */
void playerInput(); /* Bruger input  */
void loadDefaultDeck(); /* Load default deck */
void showDeck(); /* Læg kort på piles */


/* Printing operations */
void printDeck(); /* Load deck */ 
void printConsole(); /* Basic print console function */
...
/* Fjerner last pile i doubly linked list */
int removePile(){
    if(start != NULL){
        struct Pile* temp1;
...
// Denne funktion validerer brugerinput ifm. spillogik, og derefter vælger den tilhørende spillogik
    ...
    do { 
        tempCommandValue = scanf("%s[0-9a-zA-Z ]", inputCommandValue); /*
        ...
        if (strcmp(inputCommandValue, "LD") == 0) { //Load default deck function  
            loadDefaultDeck();
            initializeDeck();
            basicLD(); 
            //dealCards();
            
        } else if(strcmp(inputCommandValue, "SW") == 0) { //Show deck function
            showDeck(); 
            printDeck();
             
        } else if(strcmp(inputCommandValue, "SI") == 0) { // Split shuffle function 
            splitDeck(); 
            
        } else if (strcmp(inputCommandValue, "SR") == 0) { // Shuffle random function
            shuffleRandom();
            
        } else if (strcmp(inputCommandValue, "SD") == 0) { // Save current deck function 
            saveDeck(); 
            
        } else if (strcmp(inputCommandValue, "P") == 0) { // Start game function
            startGame(); 
             
        } else if (strcmp(inputCommandValue, "Q") == 0) { // Quit current game function
            quitCurrentGame(); 
            clearGame();
            
        } else if (strcmp(inputCommandValue, "QQ") == 0) { // Quit program
            quitGame(); 
            
        } else if (strcmp(inputCommandValue, "HELP") == 0) { // Help function 
            helpMessage(); 
            
        } /* else if (tempCommandValue[0] == 'L' && tempCommandValue[1] == 'D'  // To load "LD <filename>". Requires the use of fgets and tempCommandValue as array.
...
void printDeck() {
    ...
    while (1) { //infinite loop
        ...
        if (tempCard == NULL) {
            ...
            if (tempCard->cardRevealed = 0) { // det må ikke vises hvad der er i skjulte kort "[]"
                printf("[]");
